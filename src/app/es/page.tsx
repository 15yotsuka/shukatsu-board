...
export default function ESPage() {
  ...
  return (
    <div className="px-4 py-4 space-y-4">
      ...
      {/* Empty state */}
      {!selectedCompanyId && (
        <p className="text-[14px] text-[#8E8E93] text-center py-8">
          企業を選択してください
        </p>
      )}
        ...
        <>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="flex-1 bg-[#E8F0FE] text-[#007AFF] rounded-xl py-3 text-[14px] font-semibold ios-tap"
            >
              テンプレート
            </button>
            <button
              onClick={() => setShowCopyDialog(true)}
              className="bg-[#F2F2F7] text-[#8E8E93] rounded-xl px-4 py-3 text-[14px] font-medium ios-tap"
            >
              コピー
            </button>
          </div>
          ...
          {companyEntries.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-[17px] font-semibold text-[#1C1C1E] mb-1">設問を追加しましょう</p>
              <p className="text-[14px] text-[#8E8E93]">上のボタンから設問を追加</p>
            </div>
          ) : (
              ...
              {companyEntries.map((entry, index) => (
                <div key={entry.id}>
                  <div className="flex items-center justify-end gap-1 mb-1">
                    <span className="text-[12px] text-[#8E8E93] mr-auto">
                      設問 {index + 1}
                    </span>
                    <button
                      onClick={() => handleMoveUp(index)}
                      disabled={index === 0}
                      className="w-8 h-8 flex items-center justify-center text-[#007AFF] disabled:text-[#C7C7CC] ios-tap"
                    >
