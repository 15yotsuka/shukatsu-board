...
export function CompanyCard({ company, onTap }: CompanyCardProps) {
  ...
  return (
    <div
      ...
      className={`bg-white rounded-xl shadow-sm p-3.5 ios-tap cursor-grab active:cursor-grabbing touch-manipulation ${
        isDragging ? 'opacity-50 shadow-lg' : ''
      }`}
    >
      <p className="text-[15px] font-semibold text-[#1C1C1E] truncate">{company.name}</p>
      {company.industry && (
        <p className="text-[13px] text-[#8E8E93] mt-1 truncate">{company.industry}</p>
      )}
      {nextInterview && (
        <div className="flex items-center gap-1 mt-1.5">
          <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5 text-[#007AFF]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <p className="text-[12px] text-[#8E8E93] mt-1">{updatedDate}</p>
    </div>
  );
}
