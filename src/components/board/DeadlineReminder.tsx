...
export function DeadlineReminder() {
  ...
  // 期限超過の企業（nextDeadline < today）
  const overdueCount = companies.filter((c) => {
    if (!c.nextDeadline) return false;
    return c.nextDeadline < todayStr;
  }).length;

  // 今日の締切（nextDeadline === today）
  const todayDeadlineCount = companies.filter((c) => c.nextDeadline === todayStr).length;
  ...
  // 全て0ならバナー非表示
  if (overdueCount === 0 && todayDeadlineCount === 0 && upcoming.length === 0) return null;
          ...
          {upcoming.map(({ interview, daysUntil, company }) => {
            ...
            return (
              <div key={interview.id} className="flex items-center gap-3 px-4 py-3">
                <div className={`shrink-0 min-w-[44px] text-center px-2 py-1 rounded-lg text-[12px] font-bold ${
                  isUrgent
