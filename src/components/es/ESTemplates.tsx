    ...
    <div className="fixed inset-0 z-[70] flex items-end md:items-center justify-center">
      ...
      <div className="relative bg-card rounded-t-2xl md:rounded-2xl w-full max-w-lg animate-slide-up">
        ...
        <div className="px-4 pt-4 pb-2">
          <h2 className="text-[17px] font-bold text-center text-[var(--color-text)]">テンプレートから追加</h2>
        </div>
        ...
        <div className="p-4 space-y-2">
          {ES_TEMPLATE_QUESTIONS.map((q, i) => (
            <button
              ...
              className={`w-full text-left p-3.5 rounded-xl text-[15px] flex items-center gap-3 min-h-[44px] ios-tap transition-colors ${selected.has(i) ? 'bg-[var(--color-primary-light)]' : 'bg-[var(--color-bg)]'
                }`}
            >
              <div
                className={`w-[22px] h-[22px] rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-colors ${selected.has(i) ? 'bg-[var(--color-primary)] border-[var(--color-primary)]' : 'border-[var(--color-border)]'
                  }`}
              >
                {selected.has(i) && (
